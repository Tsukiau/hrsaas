...
export default {
  ...
  data() {
    const checkNameRepeat = async(rule, value, callback) => {
      ...
      if (this.formDate.id) {
        // 编辑
        isResult = depts.filter(item => item.pid === this.formDate.pid && item.id !== this.formDate.id).some(item => item.name === value)
      } else {
        // 新增
        isResult = depts.filter(item => item.pid === this.treeNode.id).some(item => item.name === value)
      }
   ...
   if(this.formDate.id){
    // 编辑
    isResult = depts.filter(item => item.id !== this.formDate.id).some(item => item.code === value && value)
   }else {
    // 新增
     isResult = depts.some(item => item.code === value && value)
   }
  ...
  methods: {
    ...
    // 取消 关闭触发
    btnCancel() {
      //  因为resetFields 只能重置 表单上的数据 非表单上的 比如 编辑中id 不能重置
      this.formDate = {
        name: '',
        code: '',
        manager: '',
